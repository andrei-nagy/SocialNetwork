...
<div class="container">
    ...
    <h5>Search Results - Users</h5>
    <p>{{ $users->count() }} users for '{{ request()->input('query') }}'</p>
     <div class="row">
    ...
    <div class="d-flex g-width-30 g-height-30 mr-2 ">
      @if(!empty($user->profile->image) )
       <img class="img-fluid rounded-circle" src="/storage/{{ $user->profile->image }}" alt="Image Description" > 
@else
  <img class="img-fluid rounded-circle" src="/storage/profile/vx2k9TEhkcgaRdOWKvs4lsxqOVmuzwumtwySEnvH.png" alt="Image Description">
       @endif    
       
</div>
  <div class="media-body">
                    <span class="d-block g-hidden-sm-down g-color-main g-font-weight-600 g-font-size-13 mt-1"> <a href="/profile/{{$user->id}}">{{ $user->username }}</a>  </span>
                  </div>
                </div>
    @endforeach
  </div>
...
<div class="container">
    ...
    <h5>Search Results - Tags</h5>
    <p>{{ $tags->count() }} tags for '{{ request()->input('query') }}'</p>
     <div class="row">
    @foreach($tags as $tag)
    
    <div class="col-md-2 d-flex mt-2 ">
    <div class="d-flex  g-height-30">
     
       
</div>
  <div class="media-body">
    <a class="u-tags-v1 g-color-gray-dark-v4 g-color-white--hover g-bg-gray-light-v5 g-bg-primary--hover
                g-font-size-12 g-rounded-50 g-py-4 g-px-15 mb-3" href="/tags/{{ $tag->id }}">{{ $tag->name }} <span class="badge badge-danger ml-2">{{ $tag->posts()->count() }}</span></a>
...
<hr>
<div class="container">
 <h5>Search Results - Posts</h5>
    
    <p>{{ $posts->count() }} posts for '{{ request()->input('query') }}'</p>
    
<div class="row">
  

    @foreach($posts as $post)
     <div class="masonry-grid-item col-sm-6 g-mb-30 col-md-4">
